...
<div align="right">
	<c:choose>
		<c:when test="${empty requestScope.category  }">
			<a class="btn btn-default" style="margin: 0 3%"
				href="${initParam.rootPath }/free/login/list.do?page=${requestScope.page }">목록</a>
		</c:when>
		<c:otherwise>
			<a class="btn btn-default" style="margin: 0 3%"
	...
	<c:if
		test="${requestScope.freeBoard.memberId== sessionScope.login_info.memberId }">
		...
		<c:choose>
			...
			<c:otherwise>
				<a class="btn btn-default" style="margin: 0 1%"
					href="${initParam.rootPath }/free/login/delete.do?no=${requestScope.freeBoard.no}&page=1">삭제</a>
				<p />
			</c:otherwise>
		</c:choose>
	</c:if>
...
<div id="d">
	<c:forEach items="${requestScope.freeComment }" var="comment">
		<div id="tom" style="border-bottom: 1px solid white">
			...
			<c:choose>
				...
				<c:otherwise>
					<fmt:formatDate pattern="yyyy-MM-dd HH:mm"
						value="${comment.commentRegisterTime }" />
					...
					<c:choose>
						...
						<c:otherwise>
							<span style="padding: 5px" class="secondComment"> <input
								class="second" type="button" value="답글">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							</span>
						</c:otherwise>
					</c:choose>


					<span style="padding: 5px" class="commentInfo"> <c:if
							test="${comment.commentMemberId ==sessionScope.login_info.memberId }">
							<input class="update" type="button" value="수정 ">
							<a class="delete"
								href="${initParam.rootPath }/comment/login/delete.do?id=${comment.id }&no=${comment.no}&page=${requestScope.page}&category=${requestScope.category}&search=${requestScope.search}">삭제</a>
						</c:if> <br> <c:if test="${comment.groupLevel ==1}">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</c:if> ${comment.commentContent }
					</span>
			...
			<!-- 수정 Form  -->
			<form class="updateMethod"
				action="${initParam.rootPath }/comment/login/update.do"
				method="post" hidden="false">
				<input class="scope" type="hidden"
					value="${comment.commentContent }">
				<textarea name="commentContent" rows="3" cols="50">${comment.commentContent }</textarea>
				<input class="updateCancel" type="button" value="수정취소"> <input
					type="hidden" name="id" value="${comment.id }"> <input
					type="hidden" name="no" value="${comment.no }"> <input
					type="hidden" name="page" value="${requestScope.page }"> <input
					type="hidden" name="category" value="${requestScope.category }">
				<input type="hidden" name="search" value="${requestScope.search}">
				<input class="bto" type="submit" value="수정">
			</form>

			<!-- 답글Form -->
			<form class="secondRegister"
				action="${initParam.rootPath }/comment/login/insert.do"
				method="post" hidden="">
				<hr>
				<br>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input
					class="scodCancel" type="button" value="답글 취소">└<br>&nbsp;&nbsp;
				★&nbsp;&nbsp;&nbsp;
				<textarea class="secondText" name="commentContent" rows="3"
					cols="50"></textarea>
				<input type="hidden" name="commentMemberId"
					value="${sessionScope.login_info.memberId }"> <input
					type="hidden" name="no" value="${requestScope.freeBoard.no }">
				<input type="hidden" name="groupLevel" value="1"> <input
					type="hidden" name="page" value="${requestScope.page }"> <input
					type="hidden" name="category" value="${requestScope.category }">
				<input type="hidden" name="search" value="${requestScope.search }">
				<input type="hidden" name="commentGroup"
					value="${comment.commentGroup }"> <input class="bto"
...
<!-- 댓글 -->
<form id="register"
	action="${initParam.rootPath }/comment/login/insert.do" method="post">
	<h2 style="coller: white">댓글</h2>
	<div align="center">
		<textarea class="form-control" style="width: 95%; border-bottom: 10%"
			id="comment0" name="commentContent" rows="2" cols="10"></textarea>
	</div>
	<input type="hidden" name="commentMemberId"
		value="${sessionScope.login_info.memberId }"> <input
		type="hidden" name="no" value="${requestScope.freeBoard.no }">
	<input type="hidden" name="groupLevel" value="0"> <input
		type="hidden" name="page" value="${requestScope.page }"> <input
		type="hidden" name="category" value="${requestScope.category }">
	<input type="hidden" name="search" value="${requestScope.search }">
	<div align="right">
		<input class="bto" style="" type="submit" value="등록">
	</div>
</form>

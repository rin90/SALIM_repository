...
-- [최종-버전1] big_content의 그룹으로 묶어서 각 그룹별 지출 금액 출력 
...
-- **[최종-버전2] big_content의 그룹으로 묶어서 각 그룹별 지출 금액 출력
...
-- [v.2] BigCategory와 SmallCategory별 지출합을 구해논 테이블을 join해서 대분류의 이름 알아오기 및 대분류의 내용 해당하는게 없음 0으로 초기화
select big.big_code, sum(small.total) 
from bigcategory big, 
	(select s.small_code, e.total, s.big_code
	 from smallcategory s, 
		(select code_id, nvl(sum(cash_expense),0)+nvl(sum(card_expense),0) total
